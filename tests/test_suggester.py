...
def test_suggestions_reference_anchor_gain():
    task = TaskSpec(
        ...
        robustness_checks=[{"name": "shifted-grid", "eval_command": "python shifted.py --output {output_path}"}],
        seeds=[0, 1],
    ...
    results = [
        ...
        ExperimentResult(
            ...
            run_dir="b",
            scientific_checks=[
                ScientificCheckResult(name="conservation", kind="constraint", status="passed"),
                ScientificCheckResult(name="shifted-grid", kind="robustness", status="pending"),
            ],
        ),
