...
from .storage import load_history, load_task, save_evidence_state, save_history, save_plan
...
def run_loop(task_path: Path, rounds: int, console: Console | None = None) -> None:
    ...
    for _ in range(max(remaining, 0)):
        ...
        if console:
            console.print(f"[bold]Round {plan.round_index}[/bold]: planned {len(plan.branches)} branch(es) / {len(plan.experiments)} experiment(s) in mode={round_mode}")
            for branch in plan.branches:
                note = branch.notes[0] if branch.notes else ""
                console.print(f"  - {branch.id}: {branch.tag} runs={branch.planned_evidence_runs} ({note})")

        results = execute_plan(task, plan, run_root, resume=True)
        historical_results = [r for entry in history.entries for r in entry.experiments]
        evidence_state = build_evidence_state(task, historical_results + results)
        save_evidence_state(run_root, plan.round_index, evidence_state)
        ...
        suggestion = build_suggestions(task, results, historical_results=historical_results)
        ...
        if console:
            ...
            if successful:
                ...
                console.print(
                    f"[green]Round {plan.round_index} done[/green]: mode={round_mode}, completed={completed}, failed={failed}, resumed={resumed}, best={best.experiment_id}, {metric_name}={best.metrics[metric_name]}, next={suggestion.next_action_type}"
                )
            else:
                console.print(f"[yellow]Round {plan.round_index} done[/yellow]: mode={round_mode}, completed={completed}, failed={failed}, resumed={resumed}, no valid metric yet")
